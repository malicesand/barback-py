        ...
        <h3>Renamed Cards (DCIM)</h3>
         {renamed.length === 0 && <div>No cards detected.</div>}
          ...
          <div style={{
            ...
            <div>
              {renamed.map(r => {
                const isLoading = !!loadingFolders[r.dcim_path];
                const isExpanded = !!expanded[r.dcim_path];
                const folders = foldersByDcim[r.dcim_path];

                return(
                  <div key={r.volume_path}
                    style={{ border: '1px solid #999', borderRadius: 8, padding: 12 }}>
                    <div style={{ fontWeight: 600, display: 'flex', gap: 8, alignItems: 'center' }}>
                      <span>{r.volume_name}</span>
                      {typeof r.folder_count === 'number' && (
                        <span style={{ marginLeft: 'auto', fontSize: 12, opacity: 0.7 }}>
                          {r.folder_count} folder{r.folder_count === 1 ? '' : 's'}
                        </span>
                      )}
                      <div style={{ marginTop: 10, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                        <button onClick={() => toggleOrLoadFolders(r)} disabled={isLoading}>
                          {isLoading
                            ? 'Loading DCIM…'
                            : folders
                              ? (isExpanded ? 'Hide folders' : 'Show folders')
